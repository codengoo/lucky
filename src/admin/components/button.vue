<template>
    <div class="">
        <button type="button" @click="$emit('onClick')"
            ...
            <span v-if="icon_center" class="text-lg flex flex-col justify-center">
                <slot name="icon_center"></slot>
            </span>
            {{ title }}
            <span v-if="icon" class="ms-2 text-lg flex flex-col justify-center">
                <slot name="icon"></slot>
            </span>
        </button>
    </div>
